...
/*   Updated: 2025/03/26 10:23:34 by mzhitnik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	allocate_struct(t_session *s, t_count *c, int i)
{
	...
	if (!s->cmds)
		return (error_msg("Command tree alloc failed", NULL, NULL), -1);
	while (i < c->cmd_nb)
	{
		...
		if (alloc_check(s->cmds[i], c, i) < 0)
			return (error_msg("Smthg wrong allocate_struct", NULL, NULL), -1);
		i++;
	}
	return (1);
}
